...
import { localeMap } from "./__internal__/locale-map/locale-map";
...
const DateInput = ({
  ...
  const l = useLocale();
  const { format, formats } = localeMap[l.locale()];
  const { inputRefMap, setInputRefMap } = useContext(DateRangeContext);
  const inputName = dataElement?.split("-")[0];
  const [open, setOpen] = useState(false);
  const [selectedDays, setSelectedDays] = useState(parseDate(format, value));
  ...
  const computedValue = () => {
    if (isInitialValue.current && checkISOFormatAndLength(value)) {
...
function checkISOFormatAndLength(value) {
  if (value.length !== 10 || parseISODate(value).toString === "Invalid Date") {
    return false;
  }
