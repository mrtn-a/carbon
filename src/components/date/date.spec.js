...
import { SEPARATORS } from "./__internal__/locale-map/locale-map";
...
const locales = {
  "en-GB": { locale: () => "en-GB", separator: "/" },
  de: { locale: () => "de", separator: "." },
  es: { locale: () => "es", separator: "/" },
  "en-ZA": { locale: () => "en-ZA", separator: "/" },
  "fr-FR": { locale: () => "fr-FR", separator: "/" },
  "fr-CA": { locale: () => "fr-CA", separator: "/" },
  "en-US": { locale: () => "en-US", separator: "/" },
  "en-CA": { locale: () => "en-CA", separator: "/" },
};
...
describe("Date", () => {
  ...
  const MockComponent = () => {
    const [val, setVal] = useState("02/02/2022");
    return (
      <DateInput
        value={val}
        onChange={(ev) => setVal(ev.target.value.formattedValue)}
        onFocus={onFocusFn}
      />
    );
  };
  ...
  describe("when the input value changes", () => {
    ...
    describe.each(["12/42/3213", "foo", "1/01.11"])(
      "with the DatePicker open and a invalid date of %s",
      (mockValue) => {
        it("should pass no value to the DatePicker", () => {
          wrapper = mount(<MockComponent />);
          simulateFocusOnInput(wrapper);
          expect(
            wrapper.update().find(DatePicker).props().selectedDays
          ).toEqual(new Date("02/02/2022"));
